...
### Configuration Files:
Place `.ini` files in the `ini_files` folder alongside the executable, following the `config_ID####_MID####.ini` naming format.
...
## Installation Steps
1. **Download v0.08:** Get `DataGeneration_v008.exe`.
2. **Setup Directories:** Place the executable and create the `ini_files` folder with your `.ini` configurations.
3. **Configure Settings:** Update the `.ini` files with your environment paths and preferences.
4. **Tesseract OCR:** Ensure Tesseract is installed and correctly referenced in the configuration.
